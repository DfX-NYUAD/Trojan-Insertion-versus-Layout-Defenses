	...
	while true; do
		...
		# has started at some point, and got just done, w/o errors and w/o being cancelled earlier
			...
			if [[ $vios_total == 0 ]]; then
				...
				string="Innovus Trojan insertion, Trojan \"$trojan\", TI mode \"$TI_mode\", passed without any violations."
			else
				string="Innovus Trojan insertion, Trojan \"$trojan\", TI mode \"$TI_mode\", passed with some violations."
