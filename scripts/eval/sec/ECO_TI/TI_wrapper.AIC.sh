...
start_TI() {
	...
	#
	scripts/TI_init.AIC.sh $trojan $TI_mode $dbg_files > /dev/null
	TI_init_status=$?
	...
	if [[ "$trojan" == *"aes"* ]]; then
		...
		call_invs_only scripts/TI.AIC.tcl -log TI.$trojan_TI > /dev/null &
		...
		call_vdi_invs scripts/TI.AIC.tcl -log TI.$trojan_TI > /dev/null &
		echo $! > PID.TI.$trojan_TI
	fi
