	...
	for google_team_folder in "${!google_team_folders[@]}"; do
		...
		for benchmark in $benchmarks; do
			...
			(
				...
				text+="Processing status: You have currently around $total_ongoing_runs more run(s) ongoing in total, around $total_queued_runs more run(s) queued in total, and out of those ${runs_queued[$id_internal]} more run(s) queued for this particular benchmark."
				text+=" "
...
#				# NOTE As of now, only a 0-byte power.rpt file matches here (the proper file is already in reports/power.rpt). Not sure where this file comes from.
				...
				# NOTE log files can be GBs large in case of interrupts -- might want to deprecate this
				# unzip all log files again; log files should be readily accessible for debugging
				# NOTE only mute regular stdout, but keep stderr
				unzip $folder'.zip' $folder/*.log* > /dev/null #2>&1

#				# NOTE deprecated; redundant as long as the above is active
